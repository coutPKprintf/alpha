...
public class MessageAdapterHandler {
	...
	private ConcurrentMap<String, EventProcessorWrap> epwMapOther;

	public MessageAdapterHandler(CodecFactory codecFactory) {
		this.epwMap = new ConcurrentHashMap<>();
        this.epwMapOther = new ConcurrentHashMap<>();
	...
	public void handleMessage(EventMessage eventMessage) throws Exception {
        ...
        EventProcessorWrap eventProcessorWrap;
        switch (eventMessage.getQueueInfo().getExchangeType()) {
            case DIRECT:
                eventProcessorWrap = epwMap.get(QueueInfoUtil.getBindKey(eventMessage.getQueueInfo()));
                break;
            default:
                eventProcessorWrap = epwMapOther.get(QueueInfoUtil.getBindRouteKey(eventMessage.getQueueInfo()));
        }
	...
	protected <T extends Message> void add(QueueInfo queueInfo, EventProcessor<T> processor) {
        ...
        log.info("QueueInfoUtil.getBindKey(queueInfo) = {}", QueueInfoUtil.getBindKey(queueInfo));
        ...
        switch (queueInfo.getExchangeType()) {
            case DIRECT:
                break;
            default:
                log.info("QueueInfoUtil.getBindRouteKey(queueInfo) = {}", QueueInfoUtil.getBindRouteKey(queueInfo));
                oldProcessorWrap = epwMapOther.putIfAbsent(QueueInfoUtil.getBindRouteKey(queueInfo), epw);
        }
